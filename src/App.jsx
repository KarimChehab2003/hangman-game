...
function App() {
  ...
  const checkInput = (letter) => {
    ...
    if (!wordArray.includes(letter)) setIncorrectGuesses(incorrectGuesses + 1);

    setGuessedLetters(
      guessedLetters.map((element, index) =>
        wordArray[index] === letter ? letter : element
      )
    );
  };
    ...
    <section className="bg-primary min-h-screen flex justify-center items-center px-2">
      <div className="bg-white flex flex-col sm:flex-row justify-between items-center rounded-lg shadow-md p-4 sm:p-8 w-full sm:w-fit space-y-4">
        <div className="flex flex-col justify-center items-center">
          <img
            src={hangmanPictures[incorrectGuesses]}
            alt=""
        ...
        <div className="flex flex-col justify-between items-center space-y-12">
          <p className="text-2xl sm:text-4xl font-medium tracking-[0.2em] uppercase">
            {guessedLetters.map((element) => element + " ")}
          </p>
          <p>
            Incorrect guesses:{" "}
            <span className={changeIndicator()}>{incorrectGuesses}/6</span>
          </p>
